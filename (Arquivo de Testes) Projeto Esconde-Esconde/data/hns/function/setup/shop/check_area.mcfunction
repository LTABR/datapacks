execute as @s[tag=Admin] at @s unless blocks ~ ~1 ~ ~ ~12 ~ ~ ~ ~ masked run return run tellraw @s {"text":"Local Inválido. Tente Novamente em um Local Mais Aberto.","color":"red"}
...
execute as @s[tag=Admin] at @s if blocks ~ ~1 ~ ~ ~12 ~ ~ ~ ~ masked unless entity @e[type=minecraft:block_display,tag=container] run forceload add ~ ~ ~ ~
execute as @a[tag=Admin] at @s if blocks ~ ~1 ~ ~ ~12 ~ ~ ~ ~ masked unless entity @e[type=minecraft:block_display,tag=container] run tp ~ ~12 ~
schedule function hns:setup/shop/call_at_player 1t
tag @s add awaiting
