...
summon text_display ~ ~ ~ {text:[{"text":"Spawn "},{"text":"Procuradores","bold":true,"underlined":true,"color":"dark_red"}], billboard:"center",Tags: ["spawnprocuradores"], line_width:100}
execute as @e[type=text_display,tag=spawnprocuradores] at @s run setblock ~ ~ ~ air

execute as @e[type=text_display,tag=spawnprocuradores] at @s run forceload add ~ ~
...
execute if entity @e[type=minecraft:text_display,tag=spawnescondedores] if entity @e[type=minecraft:text_display,tag=spawnprocuradores] if entity @a[tag=Admin,distance=..15] run tellraw @a[tag=Admin] ""
